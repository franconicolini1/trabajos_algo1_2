...
void _abb_in_order_acotado(ab_t* ab, abb_comparar_clave_t cmp,  bool visitar(const char *, void *, void *), void* extra, char* cota_inf, char* cota_sup) {
  if (!ab) return;

  if (cmp(ab->clave, cota_inf) > 0) {
    _abb_in_order_acotado(ab->izq, cmp, visitar, extra , cota_inf, cota_sup);
  }
  if (cmp(ab->clave, cota_inf) >= 0 && cmp(ab->clave, cota_sup) <= 0) {
    visitar(ab->clave, ab->dato, extra);
  }
  if (cmp(ab->clave, cota_sup) < 0) {
    _abb_in_order_acotado(ab->der, cmp, visitar ,extra, cota_inf, cota_sup);
  }
  
}

void abb_in_order_acotado(abb_t* arbol, bool visitar(const char *, void *, void *), void* extra , char* cota_inf, char* cota_sup) {
  _abb_in_order_acotado(arbol->raiz, arbol->comp_func, visitar, extra, cota_inf, cota_sup);
}